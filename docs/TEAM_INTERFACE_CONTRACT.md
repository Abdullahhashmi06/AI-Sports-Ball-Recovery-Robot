...
```mermaid
flowchart LR
    M1[Module 1<br/>Perception] -->|ball state, obstacle detections| M2[Module 2<br/>Navigation & Search]
...
- Ball exit direction / recovery-zone estimate with a confidence value.
...
- Camera calibration / court-map parameters supplied by integration (Person 1) once coordinate conventions are agreed (§11).
...
- Perception never issues movement, intake, or dispensing commands (see Rule R-3, §7).
...
- Approach behavior for ball collection, terminating at the agreed intake pose (see §11 open decision 6).
...
- Mechanical geometry changes that affect approach or verification must go through change control (§10) because they ripple into Module 2 and the protocol's `TELE.us` / `collect` layout.

## 5. Key Cross-Module Interfaces

Every interface below lists its **logical payload**. Where an interface crosses to the ESP32, the exact wire schema is defined in `COMMUNICATION_PROTOCOL.md` and is *not* repeated here. Payload field names marked *indicative* are frozen once §11 open decisions 1–4 are agreed.

### I-1 Perception → System state machine (rally & exit)

| | |
|---|---|
| Producer / Consumer | Module 1 (Person 2) → State machine (Person 1) |
| Trigger | On rally-state change and on exit estimation |
| Payload | `rally_state` (`RALLY_ACTIVE` / `RALLY_ENDED`), `exit_zone_estimate` { zone, confidence, ts } — zone set and units *indicative* until §11 open decision 2 |
| Notes | Drives `RALLY_ACTIVE → RALLY_ENDED → PREDICT_EXIT_ZONE` transitions (`ARCHITECTURE.md` §9). During `RALLY_ACTIVE` the robot stays at its safe/home position |
...
| Payload | `ball_state` { visible, world_pose, velocity (optional), confidence, ts }; `ball_lost` { last_known_pose, ts }; `camera_obstacles` [ { type, world_region, ts } ] — world-frame form *indicative* until §11 open decisions 1 & 3 |
| Notes | Enables approach, search hints, and fused obstacle awareness. Only Module 1 may declare a ball "found" for the `SEARCH → BALL_FOUND` transition; the trust threshold is §11 open decision 3 |
...
| Payload | `navigate_to_zone` { zone }; `search` { zone, max_extent }; `approach` { ball_pose }; `return_base` {}; `stop` {} — directive set and fields *indicative* until §11 open decision 5 |
| Notes | Directives select the *behavior*; Module 2 owns how to execute it (paths, patterns, speeds) |
...
| Notes | The exact intake pose (standoff/orientation) depends on mechanism geometry — §11 open decision 6 |
...
*Command acknowledgements (`RESP_OK` / `RESP_ERR`), sequence numbers, timeouts, and latching behavior: protocol §3, §7, §9. Response ownership on the laptop side (who consumes `RESP_*`) is §11 open decision 8.*

## 6. Data & Decision Ownership
...
| Ball "found" decision (`SEARCH → BALL_FOUND`) | Module 1 (Person 2), threshold per §11 open decision 3 | I-2 | State machine |
| Rally state | Module 1 (Person 2) | I-1 | State machine (owns transitions) |
| Exit / recovery-zone estimate | Module 1 (Person 2) — *direction estimate* | I-1 | State machine, Module 2 |
| Recovery-zone **selection** | State machine (Person 1) + Module 2 targeting | I-3, I-4 | Module 2 execution |
| Navigation state (behavior, replans, search progress) | Module 2 (Person 3) | I-4 | State machine |
| Robot pose (localization) | Module 2 (Person 3) — fuses ESP32 raw data | internal + I-4 | Module 1 (court-relative context), state machine |
...
- **R-2 — No dependence on another module's internal implementation.** An owner may refactor internals freely as long as their interface contracts (payload, authority, timing) are honored. Interface changes require §10 change control.
...
- **R-6 — The communication protocol is the single source of truth for laptop ↔ ESP32 transport.** No extra wire messages, side channels, or ad-hoc formats without a protocol version change (§13 of `COMMUNICATION_PROTOCOL.md`) approved per §10 below.
- **R-7 — One authoritative owner per datum** (§6). Producers push; consumers never "correct" authoritative data locally except by agreed reconciliation (e.g., ball count).
...
- **R-9 — Shared conventions are explicit and frozen.** Coordinate frames, units, and zone/event names are shared, agreed terms (§11); until agreed, payload forms remain *indicative* and must not be coded against.
...
| Exit-side estimation | I-1 | `exit_zone_estimate` feeds zone targeting |
...
sequenceDiagram
    ...
    M1->>SM: rally_state = RALLY_ENDED, exit_zone_estimate {zone A}
    SM->>M2: navigate_to_zone {zone A}
    loop to zone A
...
## 10. Change Control for Interfaces

1. **What counts as an interface change:** any change to a payload in §5, an ownership row in §6, an interface rule in §7, the MVP set in §8, or — for wire messages — anything in `COMMUNICATION_PROTOCOL.md`.
...
## 11. Open Interface Decisions (need team agreement before implementation)

These are deliberately **not decided here** — they are genuinely open in `ARCHITECTURE.md` / `COMMUNICATION_PROTOCOL.md` and need team agreement (and in some cases field data) before payloads in §5 can be frozen.

1. **Coordinate conventions** (world / robot / image frames, origins, units, court origin, named recovery zones) — required before I-1/I-2 world-frame payloads are meaningful. *Source: ARCHITECTURE.md "Interfaces to Agree", item 6.*
2. **Form of the exit-zone estimate** — named zone ID vs. bearing + distance from a court reference, and the minimum confidence the state machine may act on. Ties to court-boundary representation (ARCHITECTURE open decisions).
3. **Ball-world-pose derivation and the "found" threshold** — how Module 1 converts image detections to world pose (ground-plane assumption, calibration) and how many consistent detections qualify as `BALL_FOUND`. Needs dataset and field testing.
4. **Camera obstacle delivery form** — list of typed world regions (I-2) vs. another agreed form, and how Module 2 fuses them with `TELE.us` distances (Module 2 internal until the form is agreed).
5. **Search coordination split** — which search behaviors the state machine commands explicitly (directives) vs. which Module 2 chooses internally. Person 1 + Person 3 to agree so I-3/I-4 stay minimal.
6. **Intake pose definition** — standoff distance, orientation tolerance, and final stopping behavior for `approach`, dictated by Module 3 mechanism geometry (Person 4) and the verification sensor placement (protocol §14 open decision 4).
7. **Perception → reaction latency budget** — how stale `ball_state` / obstacle data may be before Module 2 must abort or stop; to be measured, not guessed.
8. **Laptop-side response ownership** — which laptop component consumes `RESP_OK` / `RESP_ERR` for each command class (motion vs. acquisition vs. reset) and how command failures surface to the state machine.

Until each item is resolved, the corresponding §5 payload stays *indicative*: do not code against it.
